...
def main(cfg_path):
    ...
    label = 'y'
    results = []
    for train_idx, test_idx in time_series_splits(df['date'], **cfg['cv']):
        tr, te = df.iloc[train_idx], df.iloc[test_idx]
        Xtr, Xte = tr[tech_cols], te[tech_cols]
        ytr, yte = tr[label], te[label]
        res = train_eval_baselines(Xtr, ytr, Xte, yte)
        results.append(res)
    ensure_dir('reports')
    with open('reports/baselines.json','w') as f:
        json.dump(results, f, indent=2)
    print("Wrote reports/baselines.json")
